# Декоратор — это функция, которая принимает другую функцию и возвращает её модифицированную версию,
# обычно добавляя какое-то поведение до, после или вместо исходной функции.
# Декоратор для подавления вывода — это специальный декоратор, который временно перенаправляет
# поток вывода (sys.stdout) в «пустышку», чтобы всё, что функция печатает (через print или sys.stdout.write), не отображалось в консоли.
